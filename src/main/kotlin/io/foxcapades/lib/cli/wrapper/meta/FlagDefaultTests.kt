...
object FlagDefaultTests {
   ...
   */
  @JvmStatic
  val Default: KClass<out FlagDefaultTest<*>>
    get() = FlagGeneralDefaultTest::class

  /**
   * [FlagDefaultTest] implementation that always returns `false`.
   */
  @JvmStatic
  val None: KClass<out FlagDefaultTest<*>>
    get() = FlagAlwaysFalseDefaultTest::class

  /**
   * [FlagDefaultTest] implementation that only considers `null` values to be
   * default.
   *
   * **Important**: This test is useless for non-nullable properties!
   */
  @JvmStatic
  val Null: KClass<out FlagDefaultTest<*>>
    get() = FlagNullDefaultTest::class

  /**
   * [FlagDefaultTest] implementation that only considers numeric zero values
   * to be defaults.
   *
   ...
   */
  @JvmStatic
  val Zero: KClass<out FlagDefaultTest<*>>
    get() = FlagZeroDefaultTest::class

  /**
   * [FlagDefaultTest] implementation that only considers numeric values equal
   * to `-1` to be defaults.
   *
   * This test will return `false` for any type that is not one of the
   * following:
   ...
   */
  @JvmStatic
  val NegativeOne: KClass<out FlagDefaultTest<*>>
    get() = FlagNegativeOneDefaultTest::class
}
